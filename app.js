...
function garantirAbaQrGestor(){
  const tabs = document.querySelector(".tabs");
  if(!tabs) return;
  let qrTab = tabs.querySelector('[data-view="qrPanel"]');
  if(!qrTab){
    const consultaTab = tabs.querySelector('[data-view="consultaPanel"]');
    qrTab = document.createElement("button");
    qrTab.className = "tab gestor-allowed";
    qrTab.dataset.view = "qrPanel";
    qrTab.textContent = "QR Code";
    qrTab.addEventListener("click", () => abrirPainel("qrPanel"));
    if(consultaTab && consultaTab.nextSibling){
      tabs.insertBefore(qrTab, consultaTab.nextSibling);
    }else{
      tabs.appendChild(qrTab);
    }
  }
  qrTab.classList.remove("staff-only", "manager-only", "hidden");
  qrTab.classList.add("gestor-allowed");
}
...
function aplicarTelaPorPerfil(){
  ...
  document.querySelectorAll(".gestor-allowed").forEach(el => {
    el.classList.remove("hidden");
    if(el.dataset?.view === "qrPanel") el.style.display = "";
  });
...
function exportarExcel(){
  if(isGestor()){ toast("Perfil gestor não exporta Excel pelo sistema. Solicite relatório à Qualidade."); return; }
...
window.abrirEditarVinculo = (colaboradorId, treinamentoId) => {
  if(!canEdit()){ toast("Perfil gestor não altera treinamentos. Solicite ajuste à Qualidade."); return; }
